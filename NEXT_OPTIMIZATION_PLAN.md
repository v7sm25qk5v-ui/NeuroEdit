...
the 168-test suite and `ruff` must stay green with no unintended user-visible change.
...
3. Keep `ruff check src tests scripts` and `python -m pytest tests/ -q` (168 tests)
...
Automation note 2026-08-24: fallback preview/playback review found that the
exporter and timeline utilities honored a 0.1 s minimum span for legacy
zero-duration slides, but the canvas active-slide lookup, canvas time-state
...
Automation note 2026-08-26: fallback timeline-duration review found that
`project_end_time()` still assigned zero-duration audio placeholders a synthetic
0.1 s span, so a legacy empty narration track could extend live preview/playback
duration after export duration had learned to ignore it. Zero-duration audio no
longer contributes to project end time; positive-duration muted tracks still
remain visible for editing. No release is indicated.

Automation note 2026-08-27: fallback export-readiness review found that export
preflight and the pre-export checklist still treated muted or zero-duration
narration placeholders as exported audio, even though the exporter does not mux
them. The UI now uses the same active-audio criteria as export muxing for
spoken-PHI warnings, narration guidance, and checklist visibility. No release is
indicated.
